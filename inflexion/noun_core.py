...
modern_plural_convert_rules = {
    ...
    rei(r"^(.*?) errant$"): {"output": lambda match: f"{convert_to_modern_plural(match.group(1)) if is_singular(match.group(1)) else match.group(1)} errant"},
    rei(r"^(.*?)-(above|across|after|among|around|athwart|at|before|behind|below|beneath|besides?|between|betwixt|beyond|but|by|during|except|for|from|into|near|of|onto|since|till|to|under|until|unto|upon|with)$"): {"output": lambda match: f"{convert_to_modern_plural(match.group(1)) if is_singular(match.group(1)) else match.group(1)}-{match.group(2)}"},
    rei(r"^(.*?) (above|across|after|among|around|athwart|at|before|behind|below|beneath|besides?|between|betwixt|beyond|but|by|during|except|for|from|into|near|of|onto|since|till|to|under|until|unto|upon|with)$"): {"output": lambda match: f"{convert_to_modern_plural(match.group(1)) if is_singular(match.group(1)) else match.group(1)} {match.group(2)}"},
...
plural_recognize_rules = {
    rei(r"^(?:(.+)os|(.+[aeiou])os|(.+[aeiou])ys|(.*)men|(.*)pox|(.*?)'s?|(.+)ies|(.+)oes|(.+)oux|(.+)xes|(.+)zoa|(.+[^ns])ses|(.+[aeio])ses|(.+[cs])hes|(.+[rnlpw])ses|(.*)bass|(.*)cows|(.*)data|(.*)deer|(.*)feet|(.*)fish|(.*)fowl|(.*)kine|(.*)mice|(.+)ceps|(.+)eaus|(.+)eaux|(.+)ieus|(.+)ieux|(.+)nges|(.+)sses|(.+)uses|(.+)zzes|(.+[^aeoui][aeoui])ches|(.+[aeo])lves|(.+[aeo])uses|(.+[aiy])nges|(.+[aiy])nxes|(.*)bemas|(.*)craft|(.*)elves|(.*)fowls|(.*)geese|(.*)hedra|(.*)helia|(.*)hertz|(.*)hoofs|(.*)lives|(.*)novae|(.*)novas|(.*)sheep|(.*)teeth|(.*)wives|(.+)-oxen|(.+)arves|(.+)eaves|(.+)nives|(.+)quies|(about|above|across|after|among|around|athwart|at|before|behind|below|beneath|besides?|between|betwixt|beyond|but|by|during|except|for|from|into|in|near|off|of|onto|on|out|over|since|till|to|under|until|unto|upon|with) them|(.*)basses|(.*)bemata|(.*)buzzes|(.*)dramas|(.*)edemas|(.*)fizzes|(.*)fuzzes|(.*)genera|(.*)hooves|(.*)humans|(.*)jazzes|(.*)knives|(.*)leaves|(.*)lemmas|(.*)loaves|(.*)lumens|(.*)lumina|(.*)nuclei|(.*)people|(.*)staffs|(.*)staves|(.*)stomas|(.*)strata|(.*)umbrae|(.*)umbras|(.*)wolves|(.+)theses|(.+)trices|(.+)trixes|(.*)bacilli|(.*)edemata|(.*)frizzes|(.*)hedrons|(.*)helices|(.*)helions|(.*)lemmata|(.*)measles|(.*)persons|(.*)shelves|(.*)siemens|(.*)stomata|(.*)thieves|(.*)brethren|(.*)brothers|(.*)children|(.*)legomena|(.*)menschen|(.*)sarcomas|(.*)stratums|(.*)phenomena|(.*)sarcomata|(.*)wildebeest|(.*)proboscides|(.*)proboscises|(.*)wildebeests|(.+)star generals)$"): {},
...
singular_recognize_rules = {
    rei(r"^(?:(.+)o|(.+)x|(.+)y|(.+)z|(.+[aeiou])o|(.+[aeiou])y|(.+[cs])h|(.+)oe|(.+)ss|(.+)us|(.+[aeio])se|(.+[aeo])lf|(.+[aiy])nx|(.+[rnlpw])se|(.*)cow|(.*)elf|(.*)man|(.*)pox|(.*?)'s?|(.+)-ox|(.+)arf|(.+)eaf|(.+)eau|(.+)ieu|(.+)nge|(.+)oux|(.+)quy|(.+[^aeoui][aeoui])che|(.+[^ns])sis|(.+[aeo])use|(about|above|across|after|among|around|athwart|at|before|behind|below|beneath|besides?|between|betwixt|beyond|but|by|during|except|for|from|into|in|near|off|of|onto|on|out|over|since|till|to|under|until|unto|upon|with) it|(.*)bass|(.*)bema|(.*)buzz|(.*)deer|(.*)fish|(.*)fizz|(.*)foot|(.*)fowl|(.*)fuzz|(.*)hoof|(.*)jazz|(.*)leaf|(.*)life|(.*)loaf|(.*)nova|(.*)wife|(.*)wolf|(.+)ceps|(.+)nife|(.+)trix|(.+)zoon|(.*)child|(.*)craft|(.*)datum|(.*)drama|(.*)edema|(.*)frizz|(.*)genus|(.*)goose|(.*)helix|(.*)hertz|(.*)human|(.*)knife|(.*)lemma|(.*)lumen|(.*)mouse|(.*)sheep|(.*)shelf|(.*)staff|(.*)stave|(.*)stoma|(.*)thief|(.*)tooth|(.*)umbra|(.*)hedron|(.*)helion|(.*)mensch|(.*)person|(.+)thesis|(.*)brother|(.*)measles|(.*)nucleus|(.*)sarcoma|(.*)siemens|(.*)stratum|(.*)bacillus|(.*)legomenon|(.*)proboscis|(.*)phenomenon|(.*)wildebeest|(.+)star general)$"): {},
...
def convert_to_singular(word, verbose=False):
    ...
    if is_singular(word, verbose=verbose):
        if verbose: print('is_singular(word, verbose=verbose)')
        return word
